...
function TeamAvatar({
  ...
  info?: { shortname: string; img: string };
  name: string;
}) {
...
function MatchCard({ match }: { match: Match }) {
  ...
  return (
    <div className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-[#FF6B00]/40 hover:bg-white/[0.07] hover:shadow-xl hover:shadow-[#FF6B00]/10">
      {/* Top edge glow */}
      ...
      <div className="mb-4 flex items-center justify-between gap-2">
        ...
        <span className="shrink-0 text-[10px] font-black uppercase tracking-[0.2em] text-slate-600">vs</span>
...
function StatusMessage({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-white/10 bg-white/[0.04] py-20 text-center backdrop-blur-xl">
      <span
...
export default function LiveMatches() {
  ...
  return (
    <section id="live-matches" className="relative overflow-hidden bg-[#050B18] px-4 py-16 sm:px-6 md:py-24 lg:px-8">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        {loading && (
          <>
            <div className="flex gap-4 overflow-x-auto pb-2 sm:hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="w-[300px] flex-none"><SkeletonCard /></div>
              ))}
            </div>
        ...
        {/* Error */}
        {!loading && error && (
          <StatusMessage icon={WifiOff} message={error} accent="#FF6B00" />
        )}
        ...
        {!loading && !error && matches.length > 0 && (
          <>
            ...
            <div className="flex gap-4 overflow-x-auto pb-3 sm:hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {matches.map((match) => (
                <div key={match.id} className="w-[300px] flex-none snap-start">
